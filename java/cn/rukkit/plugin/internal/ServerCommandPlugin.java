...
//    }
    ...
    class ServerManagerCallback implements ServerCommandListener {
            ...
            try {
                switch (args[0].toLowerCase()) {
                    case "create":
                        ...
                        Integer port = args.length >= 3 ? Integer.valueOf(args[2]) : null;
                        System.out.println("Created: " + manager.create(args[1], port));
                        break;
                    case "start":
                        if (args.length < 2) { System.out.println("Usage: server start <target> [console|background]"); return; }
                        Boolean console = null;
                        if (args.length >= 3) console = !args[2].equalsIgnoreCase("background");
                        System.out.println(manager.startTargets(args[1], console));
                        break;
                    case "stop":
                        if (args.length < 2) { System.out.println("Usage: server stop <target>"); return; }
                        System.out.println(manager.stopTargets(args[1]));
                        break;
                    case "restart":
                        if (args.length < 2) { System.out.println("Usage: server restart <target>"); return; }
                        System.out.println(manager.restartTargets(args[1]));
                        break;
                    case "list":
                        System.out.print(manager.list(args.length >= 2 ? args[1] : "all"));
                        break;
                    case "info":
                        if (args.length < 2) { System.out.println("Usage: server info <name>"); return; }
                        System.out.println(manager.info(args[1]));
                        break;
                    case "delete":
                        if (args.length < 2) { System.out.println("Usage: server delete <name> [force]"); return; }
                        System.out.println(manager.delete(args[1], args.length >= 3 && "force".equalsIgnoreCase(args[2])));
                        break;
                    case "select":
                    case "use":
                        if (args.length < 2) { System.out.println("Usage: server select <target>"); return; }
                        System.out.println(manager.select(args[1]));
                        break;
                    case "selected":
                    ...
                    case "exec":
                        ...
                        System.out.println(manager.sendTargets(args[1], join(args, 2)));
                        break;
                    case "broadcast":
                        if (args.length < 2) { System.out.println("Usage: server broadcast <command...>"); return; }
                        System.out.println(manager.sendTargets("all", join(args, 1)));
                        break;
                    case "publish":
                        if (args.length < 2) { System.out.println("Usage: server publish <target> <publish command...>"); return; }
                        System.out.println(manager.sendTargets(args[1], join(args, 2)));
                        break;
                    case "help":
                        printHelp();
        ...
        private void printHelp() {
            ...
            System.out.println("  server exec <target> <command...>");
            System.out.println("  server broadcast <command...>");
            System.out.println("Targets: all, selected, name, 2, 3-8, 2,5,7, or combinations.");
            System.out.println("Example: server broadcast publish start");
            System.out.println("Example: server send 3-8 publish start");
        }
    }
    ...
    public void onLoad() {
        ...
        mgr.registerServerCommand(new ServerCommand("server", "Create/start/stop/restart/list child Rukkit servers.", 3, new ServerManagerCallback(), this));

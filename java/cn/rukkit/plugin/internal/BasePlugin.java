...
public class BasePlugin extends InternalRukkitPlugin implements EventListener {
    ...
    public void onPlayerJoinTip(PlayerJoinEvent event) {
        event.getPlayer().getRoom().connectionManager.broadcastServerMessage(Rukkit.getConfig().notification("rukkit.playerJoin", LangUtil.getString("rukkit.playerJoin"), "playerName", event.getPlayer().name, "serverName", Rukkit.getConfig().serverUser, "roomId", event.getPlayer().getRoom().roomId, "serverPort", Rukkit.getConfig().serverPort));
    ...
    public void onPlayerLeaveTip(PlayerLeftEvent event) {
        event.getPlayer().getRoom().connectionManager.broadcastServerMessage(Rukkit.getConfig().notification("rukkit.playerLeft", LangUtil.getString("rukkit.playerLeft"), "playerName", event.getPlayer().name, "reason", event.getReason(), "serverName", Rukkit.getConfig().serverUser, "roomId", event.getPlayer().getRoom().roomId));
        if (event.getPlayer().getRoom().isGaming()) {
            event.getPlayer().sendTeamMessage(Rukkit.getConfig().notification("rukkit.playerSharingControlDueDisconnected", LangUtil.getString("rukkit.playerSharingControlDueDisconnected"), "playerName", event.getPlayer().name, "serverName", Rukkit.getConfig().serverUser));
        }
    ...
    public void onPlayerReconnected(PlayerReconnectEvent event) {
        event.getPlayer().getRoom().connectionManager.broadcastServerMessage(Rukkit.getConfig().notification("rukkit.playerReconnect", LangUtil.getString("rukkit.playerReconnect"), "playerName", event.getPlayer().name, "serverName", Rukkit.getConfig().serverUser, "roomId", event.getPlayer().getRoom().roomId));
